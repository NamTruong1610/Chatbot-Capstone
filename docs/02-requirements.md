...
| FR-API-05 | A business can be added at runtime over HTTP: `POST /api/crawl/site` (key-guarded, FR-API-02) crawls a URL, ingests it, and registers the domain, returning immediately while the crawl runs as a background task (`GET /api/crawl/site/{domain_id}` reports status). The raw crawl JSON is persisted (FR-CRAWL-09) and can be re-ingested via `corpus_path` without a live crawl. `GET /api/domains` lists every queryable business (endpoint-added plus CLI-ingested, reconciled from fingerprints). The chat endpoint serves any ready domain via a cached per-domain pipeline; an uningested domain returns 404. With no admin token configured the write endpoints fail closed (503). | M | A+ app (Phase 9) |
| FR-API-06 | Private staff content can be added at runtime: `POST /api/ingest/private` (key-guarded, FR-API-02) ingests pasted `{domain_id, title, text}` as `access_level=private` — **appended** to the domain's existing index (never a rebuild, so public content is preserved), labelled via the same explicit-override path as the file-based private corpus. Synchronous (no crawl). Isolation holds on this path exactly as the file path: a customer query never retrieves the content, a staff query does (proven with a tracer). An uningested domain returns 404. **Level 1** (the role toggle stands in for authenticated staff; real per-staff auth + write isolation is Level 2, OD-18). | M | RQ2 (Phase 11) |
| FR-API-07 | Past conversations are browsable and resumable: `GET /api/conversations?domain_id=&role=` lists them for that scope — `{session_id, domain_id, role, title, preview, updated_at, message_count}` — most-recently-active first, empties excluded, `title` = the first user message truncated (no LLM). The UI resumes one by adopting its `session_id` and loading its messages (`GET /api/chat/conversation/{id}`), so a follow-up continues it. A read (no key). | S | Conversation history (Phase 12) |
